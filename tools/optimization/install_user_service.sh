...
systemd-analyze --user verify \
    "${SYSTEMD_SOURCE}/mambapose-optimization.service" \
    "${SYSTEMD_SOURCE}/mambapose-optimization-observer.service" \
    "${SYSTEMD_SOURCE}/mambapose-optimization-observer.timer"
...
install -m 0644 "${SYSTEMD_SOURCE}/mambapose-optimization.service" \
    "${USER_UNIT_DIR}/mambapose-optimization.service"
install -m 0644 "${SYSTEMD_SOURCE}/mambapose-optimization-observer.service" \
    "${USER_UNIT_DIR}/mambapose-optimization-observer.service"
install -m 0644 "${SYSTEMD_SOURCE}/mambapose-optimization-observer.timer" \
...
"${REPO_ROOT}/.venv/bin/python" - \
    ...
    "${SYSTEMD_SOURCE}/mambapose-optimization.service" \
    "${SYSTEMD_SOURCE}/mambapose-optimization-observer.service" \
    "${SYSTEMD_SOURCE}/mambapose-optimization-observer.timer" <<'PY'
import hashlib
import json
