...
- `poll_cooperative_stop(control_path: Path, *, safe_boundary: StageSafeBoundary) -> CooperativeStopRequest | None`
- `StopResumeAuthority = TrainingResumeAuthority | StatelessStageInputAuthority`
- `write_stop_acknowledgement(request: CooperativeStopRequest, resume: StopResumeAuthority) -> StopAcknowledgement`
...
Resume tests use synthetic, source-attested tensors only. They reject mismatched manifest/init/config/seed/role/order prefix, non-finite optimizer state, missing RNG state, unvalidated checkpoint, and a fourth retained checkpoint. Generic cooperative-stop tests inject a request into training and stateless CUDA workers. Training acknowledges only after the next safe optimizer/CUDA synchronization boundary, discards all uncommitted partial-epoch effects, and names the last already validated epoch checkpoint (or run init before epoch 0). Profile/export/Stage-A/evaluation/latency acknowledge at a declared sample/iteration/CUDA synchronization boundary, atomically discard every partial staging artifact, name the exact authenticated input runtime, and restart the whole stage. Unsafe-boundary acknowledgement, a changed input/resume SHA, stale request, partial output publication, timeout, or continuing into another unit of work fails closed. Every acknowledged stop exits transiently.
...
Tests assert one immutable DAG/SHA begins by capturing/revalidating environment authority and atomically building/public-validating all ten run-init documents, then predeclares bounded W8, exactly six primary baseline/no-PIF seed0-2 preflight+training chains, both indivisible seed3/4 preflight+training pair chains, every baseline/no-PIF/W8 export-or-runtime→profile→dual-mode-evaluation→latency chain, W8 derivatives, comparison, and final audit; runtime results change activation/disposition state, never the plan. Every training node depends on its own role/seed order and real-model repeatability preflights; every latency node depends on the exact runtime/profile authority and a fresh lease and is invalidated by an overlapping contention interval. Task 3 does not fabricate the Task-4 screen schemas or producers: until Task 4 registers those exact validators/runners, attempting the screen returns permanent protocol failure and cannot activate W8. Dormant seed3/4 preflight+training chains activate together only from a validated escalation admission; no conditional training can bypass its own preflight. One non-blocking canonical controller lock prevents CLI/service or two services from mutating the campaign concurrently. The observer is read-only. State/event writes are atomic and append-only. Exit 75 is transient contention; exit 78 is permanent protocol failure; retry counters and deadlines persist across systemd restarts. Every stage revalidates the Task-2 `EnvironmentAuthority` and exact run-init authority; drift stops before process launch.

Contention tests introduce a foreign GPU owner after admission during both training and every stateless CUDA stage. The controller must revalidate the lease on every heartbeat, write an authenticated stop request, and require the worker to acknowledge only after the next stage-specific safe CUDA synchronization boundary. Training names the last validated pre-contention epoch checkpoint (or run init before epoch 0) and discards the partial epoch. Profile/export/Stage-A/evaluation/latency name their exact authenticated input runtime, publish no partial artifact, and restart the whole stage. The controller exits transiently and later resumes/restarts from that authority. An unsafe/invalid/stale acknowledgement, partial output, or timeout fails closed and terminates the worker rather than continuing contaminated work. Any latency interval overlapping contention is invalid. Tests also cover controller-lock overlap, restart-persistent retry counters, immutable-DAG mutation, and activation of only predeclared branches.
...
The immutable DAG order is: environment capture, all ten run-init documents, W8 bounded screen, its independent disposition gate, primary role/seed order+full-model repeatability preflights and six interleaved trainings, exact runtime/export→profile→dual-mode-evaluation→latency chains, three-seed comparison, dormant seed3/4 full evidence branches, final comparison, and audit. Each conditional seed branch predeclares baseline order/repeatability preflight→train→safe runtime→profile→dual-mode evaluation→latency, no-PIF preflight→train→pruned export→profile→dual-mode evaluation→latency, and—only when the global bounded screen admitted W8—a no-PIF-parent-specific W8 Stage-A→convert→complete export→profile→dual-mode evaluation→latency child. Task 3 invokes the Task-2 run-init producer and requires all documents to bind the same frozen clean HEAD/manifest/environment before any candidate stage. Escalation activates both seed3/4 branches together, but each conditional node remains blocked until its own declared dependencies validate. Task 3 owns immutable stage/dependency names and explicit unregistered-runner failure only; Task 4 supplies and wires the strict screen result/disposition producers and validators before launch. A validated candidate failure marks W8 `disabled` and proceeds to no-PIF formal training, while a missing runner/schema/provenance failure is permanent and cannot be reclassified as a candidate result. A primary deterministic/provenance failure stops the campaign. Hold one canonical campaign-controller lock for every state mutation. During every CUDA stage, revalidate the GPU lease/foreign owners on every heartbeat; on contention use the Task-2 generic cooperative-stop channel, discard uncommitted training or stateless-stage work, and resume/restart only from the acknowledged pre-contention checkpoint or exact input-runtime authority. Invalidate affected latency. Before each stage, rehash the exact `.venv`/interpreter/package/native-module and run-init authorities and refuse any drift.
...
- `convert_no_pif_w8(model: nn.Module, parent: FormalW8ParentAuthority, stage_a: FormalW8StageAResult) -> NoPIFW8Result`
...
Audit tests reject dirty/wrong source, missing required stages, terminal nonzero service result, unexplained or unbounded restarts, held shared lock, incomplete/non-validated 300-epoch results, wrong checkpoint retention, missing order hashes, different paired initialization/protocol, incomplete COCO authority, missing flip mode, malformed comparison/escalation, derivative without admission, missing or drifted formal profile/range/operation authority for any evaluated runtime, and any absolute public artifact path. The only optional-stage exception is a public parent-specific derivative-disabled disposition: the auditor verifies its exact no-PIF parent binding, candidate-versus-protocol failure classification, absence of a W8 paired/Pareto row, and an otherwise complete baseline/no-PIF lineage; it neither treats that child as silently missing nor blocks the mandatory result. A restart is admissible only when it maps one-to-one to a recorded exit-75 contention event, authenticated cooperative-stop acknowledgement, the exact Task-2 `StopResumeAuthority`, persistent retry counter/deadline, lease/foreign-owner chronology, and eventual success. For training that authority is the last pre-contention epoch checkpoint or run init; for profile/export/Stage-A/evaluation/latency it is the exact input runtime plus proof that the whole partial stage output was abandoned. Protocol/permanent failures and any unmatched restart remain forbidden.
...
5. For every baseline/no-PIF runtime and each admitted W8 derivative, produce the exact export-or-runtime authority, authenticated train-only profile/ranges/operation inventory, complete flip/no-flip evaluation, and lease-bound latency result; no comparison or handoff may skip one of these nodes.
6. Compare seeds 0-2. If and only if the CI intersects 0.1, activate both seed3/4 full branches: run every role/seed's order and repeatability preflights, train, build baseline runtime/no-PIF pruned export, profile/range/inventory, evaluate both modes, measure latency, and—if globally W8-admitted—run each no-PIF parent's Stage-A and complete W8 derivative pipeline. Recompute only after all required five-seed formal evaluation authorities validate.
