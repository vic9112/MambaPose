...
Add tests proving that the parser rejects unknown or missing keys, duplicate run IDs, absolute/traversing paths, non-hex hashes, booleans as seeds, duplicate seeds, any primary set other than `(0, 1, 2)`, any conditional set other than `(3, 4)`, epochs other than 300, persistent workers, baseline/no-PIF config asymmetry outside the declared PIF mode and run identity, and an initialization path placed in an output field.
...
Use frozen dataclasses and explicit key sets. Resolve every repository-relative path and verify it remains under the repository root. Require the exact initialization SHA above. Require ten run rows: baseline/no-PIF for seeds 0-4, with only seeds 3-4 marked conditional. The tracked manifest records expected output roots, never pre-invents checkpoint hashes.
